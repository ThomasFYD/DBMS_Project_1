    ...
    if request.method == 'POST':
        try:
            ...
            for idx, field in enumerate(selected_fields):
                ...
                ax.bar([idx - bar_width / 2], [value_region_1], bar_width, label=f'{region_1_name}' if idx == 0 else "",
                       color='b')
                ax.bar([idx + bar_width / 2], [value_region_2], bar_width, label=f'{region_2_name}' if idx == 0 else "",
                       color='r')
...
def school_comparison(request):
    if request.method == 'POST':
        try:
            ...
            input_school = schools_data.get('input_school', {})
            school_name_one = input_school.get('school_input_1')
            school_name_two = input_school.get('school_input_2')

            if (not school_1_data or not school_2_data) and (not school_name_one or not school_name_two):
                return JsonResponse({"error": "Please select two schools.Or input two schools name. "}, status=400)

            # 获取学校ID并查询数据库
            try:
                if school_name_one and school_name_two:
                    school_1 = SchoolData.objects.filter(name=school_name_one).first()
                    school_2 = SchoolData.objects.filter(name=school_name_two).first()
                    if not (school_1 and school_2):
...
def search_school(request):
    ...
    try:
        schools = SchoolData.objects.filter(name=query)
    except:
        return JsonResponse({"error": "not school name"}, status=400)
    if not schools:
        return JsonResponse({"error": "not school name"}, status=400)
    return JsonResponse({'schools': list(schools.values('district_name', 'school_address_city', 'name'))})
